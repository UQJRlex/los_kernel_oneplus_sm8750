...
#ifdef OPLUS_FEATURE_CHG_BASIC
#define TRIG_GPIO_NUM			3
enum trig_partition_id {
	TRIG1_PATTITION_ID = 1,
	TRIG2_PATTITION_ID = 2,
	TRIG3_PATTITION_ID = 3,
};
enum trig_effect_id {
	TRIG3_EFFECT_ID = 2,
	TRIG2_EFFECT_ID = 3,
...
#define RICHTAP_MAX_FIFO_DATA 2048
				...
				struct haptics_effect *effects, int effects_count)
{
	...
	for (i = 0; i < effects_count; i++) {
		...
		if (chip->trig_support) {
			if (effect->id == TRIG1_EFFECT_ID)
				chip->trig1_play_us = effect->fifo->play_length_us;
			if (effect->id == TRIG2_EFFECT_ID)
				chip->trig2_play_us = effect->fifo->play_length_us;
			if (effect->id == TRIG3_EFFECT_ID)
				chip->trig3_play_us = effect->fifo->play_length_us;
		}
#endif
			pos = 0;
...
static int haptics_read_batt_temp(struct haptics_chip *chip, int *val)
{
	static struct power_supply *batt_psy;
	union power_supply_propval ret = {0,};
	int rc = 0;
	...
	*val = DEFAULT_BATT_TEMP;
	...
	if (batt_psy) {
		...
		if (rc) {
			dev_err(chip->dev, "battery temp read error:%d\n", rc);
			return rc;
		}
		*val = ret.intval;
	} else {
...
static int haptics_set_vmax_mv(struct haptics_chip *chip, u32 vmax_mv)
{
	...
	if (chip->vbat_low_vmax_support) {
		if (chip->config.vbat_low_soc > 0) {
			haptics_read_batt_soc(chip, &batt_soc);
			haptics_read_batt_temp(chip, &batt_temp);
			if (batt_temp >= chip->config.vbat_low_temp
					&& batt_soc < chip->config.vbat_low_soc)
				vbat_is_low = true;
			else if (batt_temp < chip->config.vbat_low_temp
					&& batt_soc < chip->config.vbat_low_soc_cold)
				vbat_is_low = true;
		}

		if (vbat_is_low && vmax_mv > chip->config.vbat_low_fifo_vmax_mv) {
			vmax_mv = chip->config.vbat_low_fifo_vmax_mv;
			dev_dbg(chip->dev, "batt_soc=%u, batt_temp=%d, force vmax=%u\n",
						batt_soc, batt_temp, chip->config.vbat_low_fifo_vmax_mv);
		}
	}
#endif
...
static int haptics_enable_play(struct haptics_chip *chip, bool en)
{
	...
	dev_err(chip->dev, "haptics_enable_play, 0xf209=0x%x\n", val);
	...
	if (atomic_read(&status->written_done) == 1) {
		...
		if (atomic_read(&chip->richtap_mode)) {
			...
			if (chip->livetap_support)
				...
				num_rt = (int16_t)min_t(u32, RICHTAP_MAX_FIFO_DATA, haptics_get_available_fifo_memory(chip));
			if (num_rt < 0) {
			...
			while (num_rt > 0 && atomic_read(&chip->richtap_mode) && num_write > 0) {
...
static int haptics_parse_dt(struct haptics_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,vbat-low-soc-cold", &config->vbat_low_soc_cold);
	if (rc || config->vbat_low_soc_cold > 100) {
		config->vbat_low_soc_cold = 0;
	}

	rc = of_property_read_s32(node, "qcom,vbat-low-temp", &config->vbat_low_temp);
	if (rc) {
		config->vbat_low_temp = 0;
	}
	...
	chip->trig_support = of_property_read_bool(node, "oplus,trig_support");
	dev_err(chip->dev, "oplus,trig_support = %d\n", chip->trig_support);

	if (chip->trig_support) {
		chip->trig_gpio[0]= of_get_named_gpio(node,"oplus,trig1-gpio", 0);
		chip->trig_gpio[1]= of_get_named_gpio(node,"oplus,trig2-gpio", 0);
		chip->trig_gpio[2]= of_get_named_gpio(node,"oplus,trig3-gpio", 0);
		for (i = 0; i < TRIG_GPIO_NUM; i++) {
			if (gpio_is_valid(chip->trig_gpio[i])) {
				gpio_direction_input(chip->trig_gpio[i]);
				chip->trig_irq[i] = gpio_to_irq(chip->trig_gpio[i]);
				rc = devm_request_threaded_irq(chip->dev, chip->trig_irq[i],
					NULL, trig_irq_handler,
					IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "oplus_trig", chip);
				if (rc < 0) {
					dev_err(chip->dev, "trig_gpio[%d] request fail, rc=%d\n", i, rc);
					continue;
				}
			}
		}
...
static int haptics_detect_lra_frequency(struct haptics_chip *chip)
{
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	struct device_node *node = chip->dev->of_node;
	bool adt_drv_brk_duty_en = false;
	u32 autores_en_delay_cycles,autores_error_window = 0;
	u32 cl_lra_us_for_f0 = 0;
#endif
	...
	if (chip->hw_type >= HAP525_HV) {
#ifdef OPLUS_FEATURE_CHG_BASIC
		rc = of_property_read_u32(node, "qcom,autores-en-delay-cycles", &autores_en_delay_cycles);
		if (rc) {
			autores_en_delay_cycles = AUTORES_EN_DLY_7_CYCLES;
		}

		rc = of_property_read_u32(node, "qcom,autores-error-window", &autores_error_window);
		if (rc) {
			autores_error_window = AUTORES_ERR_WINDOW_50_PERCENT;
		}
		val |= autores_en_delay_cycles << AUTORES_EN_DLY_SHIFT |
			autores_error_window;
#else
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	dev_err(chip->dev, "config 0x%02x=0x%02x while f0 detect\n",
			chip->cfg_addr_base + HAP_CFG_AUTORES_CFG_REG, val);
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		val = DRV_DUTY_75_PCT << DRV_DUTY_SHIFT | BRK_DUTY_75_PCT;

		adt_drv_brk_duty_en = of_property_read_bool(node, "qcom,adt_drv_brk_duty_en");
		if (adt_drv_brk_duty_en) {
			val |= ADT_DRV_DUTY_EN_BIT | ADT_BRK_DUTY_EN_BIT;
		}
#else
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	dev_err(chip->dev, "config 0x%02x=0x%02x while f0 detect\n",
			chip->cfg_addr_base + HAP_CFG_DRV_DUTY_CFG_REG, val);
#endif

#ifdef OPLUS_FEATURE_CHG_BASIC
	rc = of_property_read_u32(node, "qcom,cl_lra-period-us", &cl_lra_us_for_f0);
	if (rc) {
		cl_lra_us_for_f0 = chip->config.t_lra_us;
	}
	dev_err(chip->dev, " cl_lra_us for f0 =%u\n", cl_lra_us_for_f0);
	rc = haptics_config_openloop_lra_period(chip, cl_lra_us_for_f0);
	if (rc < 0)
		goto restore;
#else
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	/* don't need auto brake calibration */
	return 0;
#endif
...
static int richtap_load_prebake(struct haptics_chip *chip, u8 *data, u32 length)
{
	...
	if (atomic_read(&chip->direct_richtap_status)) {
		dev_dbg(chip->dev, "direct_richtap_status is 1\n");
		rc = -ENOMEM;
		goto unlock;
	}
	atomic_set(&chip->direct_richtap_status, true);
...
static void richtap_work_proc(struct work_struct *work)
{
	...
	if (chip->livetap_support)
		...
		max_write_len = RICHTAP_MAX_FIFO_DATA;
	chip->pos = 0;
	count = 0;
		...
		const struct class_attribute *attr, char *buf)
{
	...
	if (chip->play.in_calibration) {
		dev_err(chip->dev, "in calibration, wait for 500ms\n");
		msleep(500);
	}
	...
	if (!(chip->play.in_calibration)) {
		...
		*/
		...
		if (rc < 0) {
			lra_impedance = 0;
			dev_err(chip->dev, "Detect LRA impedance failed, rc=%d\n", rc);
		} else if (chip->config.measure_lra_impedance) {
			lra_impedance = chip->config.lra_measured_mohms;
		} else {
			if (chip->config.lra_min_mohms == 0 && chip->config.lra_max_mohms == 0) {
				lra_impedance = 0;
				dev_err(chip->dev, "EINVAL\n");
			} else if (chip->config.lra_min_mohms == 0) {
				lra_impedance = 0;
				dev_err(chip->dev, "Short circuit\n");
			} else if (chip->config.lra_max_mohms >= chip->config.lra_open_mohms) {
				lra_impedance = 0;
				dev_err(chip->dev, "Open circuit\n");
			} else {
				lra_impedance = (chip->config.lra_min_mohms + chip->config.lra_max_mohms) / 2;
			}
		}
		...
		lra_impedance = 0;
		dev_err(chip->dev, "still in calibration\n");
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static ssize_t vmax_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t vmax_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t cl_vmax_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t cl_vmax_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t cali_time_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t cali_time_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t t_lra_us_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t t_lra_us_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t register_write_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t fifo_vmax_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t fifo_vmax_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t lra_cal_rc_clk_cal_count_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t lra_cal_rc_clk_cal_count_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t lra_cal_cl_t_lra_us_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t lra_cal_cl_t_lra_us_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t vibrator_type_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t vibrator_type_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t livetap_support_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t livetap_support_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t trig_support_show(const struct class *c,
		const struct class_attribute *attr, char *buf)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
...
static ssize_t trig_support_store(const struct class *c,
		const struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
		...
		const char __user *buff, size_t len, loff_t *data)
{
	...
	if (kstrtouint(write_data, 0, &val))
		return -EINVAL;
...
static int haptics_boost_notifier(struct notifier_block *nb, unsigned long event, void *val)
{
	...
	case VMAX_CLAMP:
		...
		break;
#ifdef OPLUS_FEATURE_CHG_BASIC
	case BATTERY_TYPE_EVENT:
		vmax_mv = *(u32 *)val;
		if (vmax_mv) {
			chip->vbat_low_vmax_support = true;
			dev_err(chip->dev, "charger module notify battery type is silicon\n");
		}
		break;
#endif
	default:
...
static int trig_load_effect(struct haptics_chip *chip)
{
	...
	int rc, i;
	bool primitive;
	if (chip->current_irq == chip->trig_irq[0]) {
		custom_data[CUSTOM_DATA_EFFECT_IDX] = TRIG1_EFFECT_ID;
		chip->current_play_us = chip->trig1_play_us;
	} else if (chip->current_irq == chip->trig_irq[1]) {
		custom_data[CUSTOM_DATA_EFFECT_IDX] = TRIG2_EFFECT_ID;
		chip->current_play_us = chip->trig2_play_us;
	} else {
		custom_data[CUSTOM_DATA_EFFECT_IDX] = TRIG3_EFFECT_ID;
		chip->current_play_us = chip->trig3_play_us;
	}
	...
	play->vmax_mv = effects[i].vmax_mv;
	dev_err(chip->dev, "upload %s effect %d, vmax=%d\n", primitive ? "primitive" : "predefined",
			effects[i].id, play->vmax_mv);
...
static int enable_memory_trig_effect(struct haptics_chip *chip)
{
	int rc;
	if (!chip) {
		dev_err(chip->dev,"enable_memory_trig_effect chip null.\n");
		return -EFAULT;
	}
	mutex_lock(&chip->play.lock);
	...
	rc = haptics_enable_play(chip, true);
	if (rc < 0) {
		dev_err(chip->dev, "enable haptics_enable_play failed rc=%d\n",rc);
		goto unlock;
	}
	usleep_range(chip->current_play_us, (chip->current_play_us) + 1);
	rc = haptics_enable_play(chip, false);
	if (rc < 0) {
		dev_err(chip->dev, "disable haptics_enable_play failed rc=%d\n",rc);
		goto unlock;
	}
unlock:
	mutex_unlock(&chip->play.lock);
...
static void sw_trig_work_proc(struct work_struct *work)
{
	int rc;
	struct haptics_chip *chip  = container_of(work, struct haptics_chip, sw_trig_work.work);
	if (!chip) {
		dev_err(chip->dev,"sw_trig_work_proc null.\n");
		return;
	}
	rc = enable_memory_trig_effect(chip);
	if (rc < 0) {
		dev_err(chip->dev, "enable_memory_trig_effect failed rc=%d\n",rc);
	}
}

static int sw_trig_init(struct haptics_chip *chip)
{
	if (!chip) {
		dev_err(chip->dev,"sw_trig_init	chip null.\n");
		return -EFAULT;
	}
	INIT_DELAYED_WORK(&chip->sw_trig_work, sw_trig_work_proc);
	return 0;
}
#endif
...
static int __maybe_unused haptics_suspend(struct device *dev)
{
	...
	if(chip->trig_support) {
		if (device_may_wakeup(dev)) {
			for (i = 0; i < TRIG_GPIO_NUM; i++) {
				if (gpio_is_valid(chip->trig_gpio[i]) && chip->trig_irq[i] > 0)
					enable_irq_wake(chip->trig_irq[i]);
			}
		}
	}
#endif
...
static int __maybe_unused haptics_resume(struct device *dev)
{
	...
	if(chip->trig_support) {
		if (device_may_wakeup(dev)){
			for (i = 0; i < TRIG_GPIO_NUM; i++) {
				if (gpio_is_valid(chip->trig_gpio[i]) && chip->trig_irq[i] > 0)
					disable_irq_wake(chip->trig_irq[i]);
			}
		}
	}
#endif
